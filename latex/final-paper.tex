...
To update values for the weights, we use the following algorithm where where $s$ and $a$ are the previous state and action, $s'$ is the new state, $r$ is the observed reward, $\delta$ is the discount factor, and $\alpha$ is the learning rate:
\begin{algorithm}
  \begin{algorithmic}
    \Procedure{updateWeights}{$s, a, s', r$}
        \State{$difference \gets \alpha(r + \delta\max_{a'}Q(s', a') - Q(s,a))$}
        \For{i = 1:n}
